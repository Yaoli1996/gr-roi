...
namespace gr {
  namespace roi {
    ...
    file_source_roi::make(size_t itemsize, const char *filename, bool is_tx_file, int role)
    {
      return gnuradio::get_initial_sptr
        (new file_source_roi_impl(itemsize, filename, is_tx_file, role));
    }

    /*
     * The private constructor
     */
    file_source_roi_impl::file_source_roi_impl(size_t itemsize, const char *filename, bool tx_file, int role)
      : gr::sync_block("file_source_roi",
              gr::io_signature::make(0, 0, 0),
              gr::io_signature::make(1, 1, itemsize)),
        itemsize(itemsize), fp(0), new_fp(0), updated(false), tx_file(tx_file), cnt(0), is_add_sob(true), _check_cnt(0), _role(role)

    {
        if (_role == 0) { // 0号消息通道订阅和发布
            d_port = pmt::mp("msg_status_file");
            message_port_register_in(d_port);
            message_port_register_out(pmt::mp("msg_status_file_1"));
            message_port_register_out(pmt::mp("msg_status_file_2"));
            message_port_register_out(pmt::mp("msg_status_file_3"));
        } else { // 1,2,3号消息通道订阅
            char str[20];
            sprintf(str, "msg_status_file_%d", _role);
            d_port = pmt::mp(str);
            message_port_register_in(d_port);
        }
      ...
      void file_source_roi_impl::send_message(int role) // 给某个文件发送发射号号
      {
...
//            pmt::pmt_t msg = pmt::cons(msg_ctl, pmt::make_u8vector(0, 0));
          ...
          printf("send message %d start\n", role);

          char str[20];
          sprintf(str, "msg_status_file_%d", role);

          message_port_pub(pmt::mp(str), msg);
      ...
      void file_source_roi_impl::msg_handler(pmt::pmt_t msg)
      {
          ...
          printf("role[%d] status_file = %d\n", _role, status_file);
          printf("**********************************\n");

          _check_cnt++;
          printf("_check_cnt = %d", _check_cnt);
          if (_check_cnt != 4) return;
