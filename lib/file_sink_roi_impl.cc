...
//            std::cout << ninput_items[0] << " " << noutput_items << std::endl;
            ...
            while (ret + d_fft_size <= input_items_num) {
                ...
                if (detect_sine(first_fft_abs)) { // 如果第一段为正弦波, 并且文件中数据已经无效
                    if (ret + total_frame_len > input_items_num) { // 如果剩余的item数目不够做第二段检波的fft, 那么就从第一次fft的开始处保留到下一次work
//                    if (ret + total_frame_len - 1504 + d_fft_size > input_items_num) { // 如果剩余的item数目不够做第二段检波的fft, 那么就从第一次fft的开始处保留到下一次work
                        break;
                    }
                    std::vector<float> second_fft_abs = do_fft(in + total_frame_len - 1504);
                    if (detect_sine(second_fft_abs)) { // 如果第二段也还为正弦波, 则将这一段数据全部写入文件
//                            printf("write data index = %d, write items num = %d, input_items_num = %d, ret = %d\n", cnt++, total_frame_len - 1504 + d_fft_size, input_items_num, ret);
...
//                        int t_size = fwrite(in, sizeof(gr_complex), total_frame_len - 1504 + d_fft_size, d_fp);
                        int t_size = fwrite(in, sizeof(gr_complex), total_frame_len, d_fp);
                        rewind(d_fp);
...
//                        in = in + total_frame_len - 1504 + d_fft_size;
//                        ret += total_frame_len - 1504 + d_fft_size;
                        in = in + total_frame_len;
                        ret += total_frame_len;
                        break;
                    }
                }
